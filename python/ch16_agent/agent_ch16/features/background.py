...
class BackgroundJobStore(Protocol):
    ...
    def finish_running(
        self, job_id: str, status: str, result: ToolResult
    ) -> BackgroundJob | None: ...
...
class JobSupervisor:
    ...
    def submit(
        self, source_tool_call_id: str, tool_name: str, operation: BackgroundOperation
    ) -> str:
        ...
        with self._lock:
            ...
            worker = threading.Thread(
                target=self._run_worker, args=(job_id, operation, cancel_event), daemon=True
            )
    ...
    def _run_worker(
        self, job_id: str, operation: BackgroundOperation, cancel_event: threading.Event
    ) -> None:
...
class BackgroundDispatcher:
    ...
    def dispatch(
        ...
        if (
            definition is None
            or definition.concurrency != "background_eligible"
            or prepared.arguments is None
        ):
            return None
        ...
        job_id = self.supervisor.submit(
            ...
            lambda cancel: tools.invoke(
                PreparedToolCall(prepared.call, definition, arguments, None), context
            ),
        )
...
def register_background_job_tools(registry: ToolRegistry, supervisor: JobSupervisor) -> None:
    ...
    registry.register(
        ToolDefinition(
            "query_background_job", "查询后台任务当前状态", schema, "read", query, validate
        )
    )
    registry.register(
        ToolDefinition(
            "cancel_background_job", "取消正在运行的后台任务", schema, "write", cancel, validate
        )
    )
