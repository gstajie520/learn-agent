...
from agent_ch16.core.messages import (
    assistant_message,
    system_message,
    tool_call,
    tool_message,
...
from agent_ch16.core.tools import tool_error, tool_success
from agent_ch16.features.compaction import (
...
class RecordingSummarizer:
    ...
    def summarize(self, history):
        ...
        return CompactionSummary(
            "继续迁移第九章",
            ("按完整消息组压缩",),
            ("agent_ch16/features/compaction.py",),
            ("运行测试",),
            ("不能拆散工具消息",),
        )
...
def test_persists_result_above_utf8_threshold_and_preserves_error_metadata(tmp_path: Path):
    manager = CompactionManager(
        str(tmp_path),
        RecordingSummarizer(),
        id_generator=ids("large"),
        persist_threshold_bytes=8,
...
def test_batch_failure_removes_created_artifacts_and_never_overwrites(tmp_path: Path):
    ...
    manager = CompactionManager(
        str(tmp_path),
        RecordingSummarizer(),
        id_generator=ids("created", "conflict"),
...
def test_batch_budget_persists_largest_inline_result_first(tmp_path: Path):
    ...
    manager = CompactionManager(
        str(tmp_path),
        RecordingSummarizer(),
        id_generator=ids("largest"),
        persist_threshold_bytes=100,
        batch_budget_bytes=9,
    )
...
def test_micro_and_snip_keep_tool_pairs_atomic():
    ...
    longer = (
        user_message("head"),
        system_message("middle"),
        *exchange("middle-tool", "paired"),
        user_message("middle-2"),
        *exchange("tail-tool", "tail"),
