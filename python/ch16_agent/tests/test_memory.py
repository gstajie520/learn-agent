...
from agent_ch16.core.messages import assistant_message, tool_call, user_message
from agent_ch16.core.model import ModelReply, ModelRequest
from agent_ch16.features.memory import (
    MAX_MEMORY_BYTES,
    MemoryRecord,
    MemorySession,
    MemoryStore,
...
def test_invalid_consolidation_plan_keeps_old_collection(tmp_path: Path, output: str) -> None:
    ...
    first = record("first")
    store.add(first)
    session = MemorySession(store, consolidator=FixedConsolidator(output), consolidate_threshold=1)
