...
from agent_ch05.core.messages import tool_call
from agent_ch05.core.tools import ToolContext, ToolRegistry
from agent_ch05.features.todos import (
    MAX_TODOS,
    TODO_STALE_REMINDER,
    TODO_STATUSES,
    TodoItem,
...
def invoke_todo(tracker: TodoTracker, arguments_json: str):
    ...
    return tools.invoke(tools.prepare(tool_call("todo-call", "todo_write", arguments_json)), CONTEXT)
...
def test_three_statuses_trim_content_and_return_ascii_snapshot() -> None:
    ...
    assert result.content == '{"todos":[{"content":"\\u7f16\\u5199\\u6d4b\\u8bd5","status":"pending"}]}'
...
def test_returns_complete_stable_snapshot_and_schema() -> None:
    ...
    tools = ToolRegistry()
    tools.register(tracker.tool_definition)
    arguments = {"todos": [{"content": "第一步", "status": "in_progress"}, {"content": "ship", "status": "completed"}]}
    result = tools.invoke(tools.prepare(tool_call("todo", "todo_write", json.dumps(arguments, ensure_ascii=False))), CONTEXT)
    assert json.loads(result.content) == {"todos": [{"content": "第一步", "status": "in_progress"}, {"content": "ship", "status": "completed"}]}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (json.dumps({"todos": [{"content": f"task-{index}", "status": "pending"} for index in range(MAX_TODOS + 1)]}), "invalid_arguments"),
        ("{", "invalid_json"),
    ],
)
