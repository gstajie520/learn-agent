...
from agent_ch05.core.tools import (
    ToolContext,
    ToolDefinition,
    ToolRegistry,
    tool_success,
...
def test_ask_fails_closed_and_explicit_approval_allows() -> None:
    ...
    denied = PermissionPolicy(rules=(rule,), write_boundary=WriteBoundary(True)).decide(request(prepared))
    approval = RecordingApproval(PermissionDecision("allow", "只批准一次", "approval"))
    allowed = PermissionPolicy(rules=(rule,), approval=approval, write_boundary=WriteBoundary(True)).decide(request(prepared))
    assert denied.behavior == "deny"
    assert denied.to_tool_result().error_code == "permission_denied"
    assert allowed == PermissionDecision("allow", "只批准一次", "approval")
    assert approval.requests[0].proposed_decision == PermissionDecision("ask", "写入需要确认", "confirm-write")
...
def test_shell_defaults_to_ask_and_reads_default_to_allow() -> None:
    ...
    assert PermissionPolicy().decide(read) == PermissionDecision("allow", "没有权限规则阻止请求", "default")
