...
class AgentRunner:
    ...
    def __init__(self, model: ModelClient, tools: ToolRegistry, system_prompt: str, workspace: str, max_turns: int = 20, identity: str = "user", authorizer: ToolAuthorizer | None = None, permission_policy: PermissionPolicy | None = None, hooks: HookRegistry | None = None, tool_round_observer: ToolRoundObserver | None = None) -> None:
        if max_turns <= 0:
    ...
    async def _run(self, prompt: str) -> RunResult:
        ...
        for turn in range(1, self._max_turns + 1):
            ...
            request = ModelRequest(messages=(system_message(self._system_prompt), *self._history, *observer_guidance), tools=snapshot.openai_tools())
            ...
            if self._tool_round_observer is not None:
                ...
                self._tool_round_observer.record_tool_round(tuple(call.name for call in assistant.tool_calls))
