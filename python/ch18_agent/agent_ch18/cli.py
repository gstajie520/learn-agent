...
def main() -> int:
    """解析参数、先验证 Git 仓库，再装配 P18 Worktree Agent。"""
    parser = argparse.ArgumentParser(description="第十八章 Git Worktree 隔离协作 Agent")
    ...
    try:
        ...
        worktree_runtime = WorktreeRuntime(
            str(Path.cwd()), task_store, SubprocessGitRunner()
        )
