...
def build_agent(
    ...
    if profile in (P17, P18) and work_stealing_runtime is None:
        raise ValueError("第十七章及以后必须提供 work_stealing_runtime")
    if profile not in (P17, P18) and work_stealing_runtime is not None:
        raise ValueError("work_stealing_runtime 只适用于第十七章及以后")
    if profile in (P17, P18):
        if task_store is None or work_stealing_runtime is None:
            raise ValueError("第十八章必须提供 SQLite task_store 和 work_stealing_runtime")
        if task_store is not work_stealing_runtime.store:
            raise ValueError("第十八章 task_store 必须和 work_stealing_runtime 共享同一实例")
    if profile is P18:
        if worktree_runtime is None:
            raise ValueError("第十八章必须提供 worktree_runtime")
        if task_store is not worktree_runtime.store:
            raise ValueError("第十八章 worktree_runtime 必须和 task_store 共享同一实例")
        if work_stealing_runtime is None or work_stealing_runtime.claim_service is not worktree_runtime:
            raise ValueError("第十八章 work_stealing_runtime 必须使用同一个 worktree_runtime")
    elif worktree_runtime is not None:
        raise ValueError("worktree_runtime 只适用于第十八章")
    ...
    if profile in (P03, P04, P05, P06, P07, P08, P09, P10, P11, P12, P13, P14, P15, P16, P17, P18):
    ...
    if "subagent" in profile.capabilities:
        ...
        def child_tools_factory() -> tuple[ToolRegistry, TodoTracker]:
            ...
            if profile in (P17, P18) and work_stealing_runtime is not None:
                register_leased_task_tools(
    ...
    if profile is P18 and worktree_runtime is not None:
        for definition in worktree_runtime.lead_tool_definitions:
            tools.register(definition)
    elif profile not in (P17, P18) and task_store is not None:
    ...
    if profile in (P14, P15, P16, P17, P18):
    ...
    if profile in (P15, P16, P17, P18):
        ...
        def teammate_factory(name: str, role: str, send_definition: object) -> AgentRunner:
            ...
            return AgentRunner(
                ...
                tool_context_provider=worktree_runtime if profile is P18 else None,
            )
