...
class JsonCronStore(CronStore):
    ...
    def schedule_cron(self, input: Mapping[str, object]) -> CronJob:
        ...
        job = CronJob(
            canonical_cron_id(str(self.id_generator())),
            cron,
            prompt,
            timezone,
            recurring,
            durable,
            identity,
            next_cron_occurrence(cron, timezone, now),
            None,
        )
        with self._locked(create=True) as paths:
            ...
            if durable:
                ...
                self._write(
                    ...
                    {
                        "version": 1,
                        "jobs": [*_serialize_jobs(jobs), _serialize_job(job)],
                        "outbox": [_serialize_event(event) for event in events],
                    },
                )
            else:
    ...
    def tick(self, now_utc: dt.datetime, include_durable: bool = True) -> tuple[CronEvent, ...]:
        ...
        with self._locked(create=True) as paths:
            ...
            for job in sorted(
                (item for item in jobs if item.next_run_at_utc <= now),
                key=lambda item: (item.next_run_at_utc, item.id),
            ):
                ...
                if job.recurring:
                    updated = CronJob(
                        ...
                        job.durable,
                        job.identity,
                        next_cron_occurrence(job.cron, job.timezone, now),
                        job.next_run_at_utc,
                    )
            ...
            if any(event.durable for event in created):
                self._write(
                    ...
                    {
                        "version": 1,
                        "jobs": [_serialize_job(job) for job in durable_jobs.values()],
                        "outbox": [_serialize_event(event) for event in durable_outbox.values()],
                    },
                )
    ...
    def pending_events(self, include_durable: bool = True) -> tuple[CronEvent, ...]:
        ...
        with self._locked(create=False) as paths:
            ...
            events = ([*stored_events] if include_durable else []) + list(
                self._session_outbox.values()
            )
    ...
    def ack_event(self, event_id: str) -> bool:
        ...
        with self._locked(create=False) as paths:
            ...
            self._write(
                ...
                {
                    "version": 1,
                    "jobs": [_serialize_job(job) for job in stored_jobs],
                    "outbox": [
    ...
    def _load(self, paths: Path) -> tuple[list[CronJob], list[CronEvent]]:
        ...
        try:
            ...
            if (
                ...
                or set(payload) != {"version", "jobs", "outbox"}
...
def _serialize_job(job: CronJob) -> dict[str, object]:
    return {
        ...
        "next_run_at_utc": _iso(job.next_run_at_utc),
        "last_slot_at_utc": None if job.last_slot_at_utc is None else _iso(job.last_slot_at_utc),
    }
...
def _serialize_event(event: CronEvent) -> dict[str, object]:
    return {
        ...
        "slot_at_utc": _iso(event.slot_at_utc),
    }
...
def _parse_job(value: object) -> CronJob:
    ...
    return CronJob(
        str(value["id"]),
        validate_cron_expression(str(value["cron"])),
        str(value["prompt"]),
        validate_cron_timezone(str(value["timezone"])),
        bool(value["recurring"]),
        bool(value["durable"]),
        str(value["identity"]),
        _parse_time(value["next_run_at_utc"]),
        last,
    )
...
def _parse_event(value: object) -> CronEvent:
    ...
    return CronEvent(
        ...
        validate_cron_timezone(str(value["timezone"])),
        bool(value["durable"]),
        _parse_time(value["slot_at_utc"]),
        context_identity=str(value["identity"]),
        idempotency_key=event_id,
    )
