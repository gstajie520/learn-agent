"""第十八章 SQLite 任务仓库：保存 DAG、认领令牌和有限租约。
...
from ..features.worktrees import WorktreeBinding, WorktreeEvent, WorktreeStateError
...
class SqliteTaskStore:
    """把第十八章项目任务保存到 ``.agent_tutorial/tasks.sqlite3``。
    ...
    def reserve_worktree(self, binding: WorktreeBinding) -> WorktreeBinding:
        """在同一事务写入 reserved binding 和 reserve 审计事件。"""
        with self._transaction() as connection:
            task = self._get_existing(connection, binding.task_id)
            if task.status != "pending":
                raise WorktreeStateError("创建 Worktree 需要 pending Task")
            try:
                connection.execute(
                    """INSERT INTO worktree_bindings(
                    task_id,name,branch,relative_path,integration_ref,baseline_commit,
                    branch_tip,status,review_reason,created_at_utc,updated_at_utc
                    ) VALUES (?,?,?,?,?,?,NULL,?,?,?,?)""",
                    (binding.task_id,binding.name,binding.branch,binding.relative_path,
                     binding.integration_ref,binding.baseline_commit,binding.status,
                     binding.review_reason,_encode_time(binding.created_at_utc),_encode_time(binding.updated_at_utc)),
                )
            except sqlite3.IntegrityError as error:
                raise WorktreeStateError("Worktree task、name、branch 或 path 已经预留") from error
            self._append_worktree_event(connection, "reserve", binding)
        return binding

    def activate_worktree(self, task_id: str, branch_tip: str, occurred_at_utc: datetime) -> WorktreeBinding:
        """把 reserved binding 原子迁移为 active，并追加 create 事件。"""
        return self._transition_worktree(task_id, ("reserved",), "active", "create", branch_tip, None, occurred_at_utc, "pending")

    def keep_worktree(self, task_id: str, branch_tip: str, occurred_at_utc: datetime) -> WorktreeBinding:
        """把 completed + active binding 标记为 kept。"""
        return self._transition_worktree(task_id, ("active",), "kept", "keep", branch_tip, None, occurred_at_utc, "completed")

    def mark_worktree_needs_review(self, task_id: str, branch_tip: str | None, reason: str, occurred_at_utc: datetime) -> WorktreeBinding:
        """把 active binding 标记为 needs_review，保留人工复核原因。"""
        return self._transition_worktree(task_id, ("active",), "needs_review", "needs_review", branch_tip, reason, occurred_at_utc, "completed")

    def mark_worktree_removed(self, task_id: str, branch_tip: str, occurred_at_utc: datetime) -> WorktreeBinding:
        """把完成后的 binding 标记为 removed，历史事件仍然保留。"""
        return self._transition_worktree(task_id, ("active", "kept", "needs_review"), "removed", "remove", branch_tip, None, occurred_at_utc, "completed")

    def get_worktree_binding(self, task_id: str) -> WorktreeBinding:
        """读取指定 Task 的 Worktree 当前快照。"""
        with self._transaction() as connection:
            return self._load_worktree_binding(connection, canonical_task_id(task_id))

    def list_worktree_events(self) -> tuple[WorktreeEvent, ...]:
        """按 sequence 升序读取 append-only Worktree 审计链。"""
        with self._transaction() as connection:
            rows = connection.execute("SELECT * FROM worktree_events ORDER BY sequence").fetchall()
            return tuple(self._event_from_row(row) for row in rows)

    def claim_next_bound(self, owner: str) -> TaskClaim | None:
        """只从 active binding 的 ready Task 中自动认领。"""
        normalized_owner = normalize_owner(owner)
        with self._transaction() as connection:
            now = self._now()
            self._release_expired(connection, now)
            tasks = self._load_tasks(connection)
            active = {row["task_id"] for row in connection.execute("SELECT task_id FROM worktree_bindings WHERE status='active'")}
            by_id = {task.id: task for task in tasks}
            for task in tasks:
                if task.id in active and task.status == "pending" and all(by_id[d].status == "completed" for d in task.blocked_by):
                    return self._claim(connection, task, normalized_owner, now)
            return None

    def lookup_claim(self, claim_token: str) -> TaskClaim | None:
        """查询仍 active 的 claim；历史 token 失效时显式报错而非回落主目录。"""
        token = canonical_claim_token(claim_token)
        with self._transaction() as connection:
            known = connection.execute("SELECT task_id FROM task_claim_tokens WHERE token=?", (token,)).fetchone()
            if known is None:
                return None
            now = self._now()
            self._release_expired(connection, now)
            row = connection.execute("SELECT * FROM tasks WHERE claim_token=? AND status='in_progress'", (token,)).fetchone()
            if row is None:
                raise TaskClaimError("claim token 已知但不再 active")
            task = self._get_existing(connection, row["id"])
            lease = _parse_time(row["lease_expires_at_utc"])
            if lease is None:
                raise TaskStorageError("active claim 缺少租约截止时间")
            return TaskClaim(task, token, lease)
    ...
    def _transition_worktree(
        ...
        required_task_status: str,
    ) -> WorktreeBinding:
        """统一执行 Binding 状态更新和事件追加，任一步失败都回滚。"""
        normalized_id = canonical_task_id(task_id)
        with self._transaction() as connection:
            task = self._get_existing(connection, normalized_id)
            if task.status != required_task_status:
                raise WorktreeStateError(
                    f"任务 {task.id} 当前是 {task.status}，需要 {required_task_status}"
                )
            current = self._load_worktree_binding(connection, normalized_id)
            if current.status not in expected:
                raise WorktreeStateError(f"Worktree binding 当前是 {current.status}，不允许迁移")
            updated = WorktreeBinding(
                current.task_id,
                current.name,
                current.branch,
                current.relative_path,
                current.integration_ref,
                current.baseline_commit,
                current.branch_tip if branch_tip is None else branch_tip,
                target,
                reason,
            ...
            changed = connection.execute(
                """UPDATE worktree_bindings SET branch_tip=?,status=?,review_reason=?,updated_at_utc=?
                WHERE task_id=? AND status=?""",
                (updated.branch_tip, updated.status, updated.review_reason,
                 _encode_time(updated.updated_at_utc), updated.task_id, current.status),
            ).rowcount
            if changed != 1:
                raise WorktreeStateError("Worktree binding 状态已经发生变化")
            self._append_worktree_event(connection, action, updated)
            return updated

    @staticmethod
    def _load_worktree_binding(connection: sqlite3.Connection, task_id: str) -> WorktreeBinding:
        """把一行 SQLite 数据还原成不可变 WorktreeBinding。"""
        row = connection.execute("SELECT * FROM worktree_bindings WHERE task_id=?", (task_id,)).fetchone()
        if row is None:
            raise WorktreeStateError(f"任务没有 Worktree binding: {task_id}")
        return WorktreeBinding(
            row["task_id"], row["name"], row["branch"], row["relative_path"],
            row["integration_ref"], row["baseline_commit"], row["branch_tip"],
            row["status"], row["review_reason"], _required_time(row["created_at_utc"]),
            _required_time(row["updated_at_utc"]),
        )

    @staticmethod
    def _append_worktree_event(connection: sqlite3.Connection, action: str, binding: WorktreeBinding) -> None:
        """复制 Binding 快照到事件表，形成不依赖状态表的审计事实。"""
        connection.execute(
            """INSERT INTO worktree_events(
            action,status,task_id,name,branch,relative_path,integration_ref,baseline_commit,
            branch_tip,reason,created_at_utc) VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (action,binding.status,binding.task_id,binding.name,binding.branch,
             binding.relative_path,binding.integration_ref,binding.baseline_commit,
             binding.branch_tip,binding.review_reason,_encode_time(binding.updated_at_utc)),
        )

    @staticmethod
    def _event_from_row(row: sqlite3.Row) -> WorktreeEvent:
        """把审计行还原成 WorktreeEvent，并重新执行领域校验。"""
        return WorktreeEvent(
            row["sequence"], row["action"], row["status"], row["task_id"], row["name"],
            row["branch"], row["relative_path"], row["integration_ref"],
            row["baseline_commit"], row["branch_tip"], row["reason"],
            _required_time(row["created_at_utc"]),
        )
    ...
    def _create_schema(connection: sqlite3.Connection) -> None:
        """幂等创建任务表、Binding 表和 append-only 审计表。"""
...
def _required_time(value: object) -> datetime:
    """解析 Worktree 必填时间；NULL 直接视为持久化数据损坏。"""
    parsed = _parse_time(value)
    if parsed is None:
        raise TaskStorageError("数据库中的 Worktree 时间不能为空")
    return parsed
