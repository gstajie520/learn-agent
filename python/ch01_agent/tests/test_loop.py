...
from agent_ch01.bootstrap import build_agent
from agent_ch01.core.commands import CommandResult
from agent_ch01.core.loop import AgentLimitError
from agent_ch01.core.messages import assistant_message, tool_call
from agent_ch01.core.model import ModelReply
from agent_ch01.core.profiles import P01
...
def test_loop_executes_tool_then_returns_final_text(tmp_path):
    ...
    model = FakeModel([
        ModelReply(assistant_message(None, (tool_call("call-1", "shell", '{"command":"Write-Output 42"}'),)), "tool_calls"),
...
def test_loop_enforces_max_turns(tmp_path):
    ...
    model = FakeModel([
        ModelReply(assistant_message(None, (tool_call("a", "shell", '{"command":"pwd"}'),)), "tool_calls"),
        ModelReply(assistant_message(None, (tool_call("b", "shell", '{"command":"pwd"}'),)), "tool_calls"),
    ])
    runner = build_agent(P01, model, str(tmp_path), command_runner=FakeCommandRunner(CommandResult("ok", 0, False, False)), max_turns=2)
    try:
        runner.run("一直调用")
