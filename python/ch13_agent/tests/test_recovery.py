...
from agent_ch13.core.messages import assistant_message, system_message, user_message
from agent_ch13.core.model import (
...
from agent_ch13.features.compaction import ArtifactReference, HistoryCompactionOutcome
from agent_ch13.features.recovery import (
    CONTINUATION_PROMPT,
    CancellationToken,
...
def test_rate_limit_honors_seconds_and_http_date_and_rejects_invalid_value() -> None:
    ...
    model = ActionModel(
        [
            ModelRateLimitError(
                "限流", retry_after=(clock + timedelta(seconds=2)).strftime("%a, %d %b %Y %H:%M:%S GMT")
            ),
...
def test_overload_switches_to_fallback_after_threshold() -> None:
    ...
    manager.begin_turn()
    manager.complete(_request())
    assert [request.model for request in model.requests] == ["primary", "primary", "primary", "fallback"]
