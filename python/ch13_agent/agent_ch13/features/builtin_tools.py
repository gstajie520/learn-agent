...
def _shell_args(value: Mapping[str, Any], *, background: bool = False) -> bool:
    allowed = {"command", "run_in_background"} if background else {"command"}
    return (
        set(value).issubset(allowed)
        and isinstance(value.get("command"), str)
        and bool(value["command"])
        and ("run_in_background" not in value or value["run_in_background"] is None or isinstance(value["run_in_background"], bool))
    )


def create_shell_tool(command_runner: CommandRunner, *, background: bool = False) -> ToolDefinition:
    ...
    properties: dict[str, Any] = {"command": {"type": "string", "minLength": 1}}
    if background:
        properties["run_in_background"] = {"type": ["boolean", "null"]}
    return ToolDefinition(
        ...
        "execute",
        handler,
        (lambda value: _shell_args(value, background=background)),
        "background_eligible" if background else None,
    )
...
def create_chapter_one_tools(command_runner: CommandRunner, *, background: bool = False) -> ToolRegistry:
    """构造 P01 工具集，仅包含 shell。"""
    registry = ToolRegistry()
    registry.register(create_shell_tool(command_runner, background=background))
    return registry


def create_chapter_two_tools(
    command_runner: CommandRunner, file_system: WorkspaceFileSystem, *, background: bool = False
) -> ToolRegistry:
    """在 P01 工具集上累加四个文件工具。"""
    registry = create_chapter_one_tools(command_runner, background=background)
