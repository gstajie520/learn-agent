...
from agent_ch14.features.cron import (
    CronEvent,
    CronExpressionError,
...
def test_cron_supports_lists_ranges_steps_and_dom_dow_or() -> None:
    ...
    assert next_cron_occurrence("0 9 1 * 1", "UTC", dt.datetime(2026, 6, 1, 9, 1, tzinfo=dt.UTC)) == dt.datetime(2026, 6, 8, 9, tzinfo=dt.UTC)
...
def test_cron_event_payload_has_context_identity_and_idempotency() -> None:
    event = CronEvent(EVENT, JOB, "cron-owner", "检查 CI", "Asia/Shanghai", True, dt.datetime(2026, 6, 1, 1, tzinfo=dt.UTC), context_identity="cron-owner", idempotency_key=EVENT)
