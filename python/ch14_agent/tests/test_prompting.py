...
from agent_ch14.core.tools import ToolDefinition, ToolRegistry, tool_success
from agent_ch14.features.memory import MemoryRecord, MemorySession, MemoryStore
from agent_ch14.features.prompting import (
    DynamicPromptProvider,
    DynamicPromptRenderer,
    PromptContextError,
)
from agent_ch14.features.skills import SkillRegistry
...
def test_renderer_outputs_fixed_sections_without_private_bodies(tmp_path: Path) -> None:
    ...
    store.extend(
        (
            ...
            MemoryRecord(
                "keyboard", "未选择的键盘说明", "project", "PRIVATE UNSELECTED MEMORY"
            ),
        )
    )
...
def test_invalid_context_does_not_poison_previous_cache(
    tmp_path: Path, context: object
) -> None:
    renderer = DynamicPromptRenderer()
    tools = ToolRegistry()
    valid = renderer.render(
        identity="agent", tools=tools, workspace=str(tmp_path), context={}
    )
