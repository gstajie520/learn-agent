...
class AgentRunner:
    ...
    def run_events(self) -> RunResult | None:
        """消费一条带独立身份的运行时事件，启动没有人工 prompt 的事件回合。"""
    ...
    async def _run(self, prompt: str, runtime_event: RuntimeEvent | None = None) -> RunResult:
        ...
        context = ToolContext(
            self._workspace,
            self._identity if runtime_event is None or runtime_event.context_identity is None else runtime_event.context_identity,
            None if runtime_event is None else runtime_event.idempotency_key,
        )
        ...
        for turn in range(1, self._max_turns + 1):
            ...
            if not assistant.tool_calls:
                ...
                if self._event_pump is not None and self._event_pump.has_pending_work:
                    self._inject_runtime_events(True, allow_context_events=runtime_event is not None)
                    continue
