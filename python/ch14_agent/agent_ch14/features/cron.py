...
class CronEvent:
    ...
    def to_payload(self) -> Mapping[str, object]:
        ...
        return {"event_id": self.event_id, "job_id": self.job_id, "kind": "cron", "identity": self.identity, "prompt": self.prompt, "timezone": self.timezone, "durable": self.durable, "slot_at_utc": self.slot_at_utc.astimezone(dt.UTC).isoformat().replace("+00:00", "Z")}
...
class CronRuntime:
    ...
    def __init__(self, store: CronStore, inbox: EventInbox, *, supervisor: object | None = None, clock: CronClock | None = None, poll_seconds: float = 1.0) -> None:
        if poll_seconds <= 0:
    ...
    def tool_definition(self) -> ToolDefinition:
        ...
        return ToolDefinition("schedule_cron", "创建一个周期或一次性 Cron 计划", {"type": "object", "properties": {"cron": {"type": "string"}, "prompt": {"type": "string"}, "timezone": {"type": "string"}, "recurring": {"type": "boolean"}, "durable": {"type": "boolean"}}, "required": ["cron", "prompt", "timezone", "recurring", "durable"], "additionalProperties": False}, "write", handler, _schedule_args)
...
def _schedule_args(value: Mapping[str, object]) -> bool:
    ...
    return set(value) == {"cron", "prompt", "timezone", "recurring", "durable"} and all(isinstance(value[name], str) and bool(str(value[name]).strip()) for name in ("cron", "prompt", "timezone")) and isinstance(value["recurring"], bool) and isinstance(value["durable"], bool)


def _job_payload(job: CronJob) -> dict[str, object]:
    """把 CronJob 转为稳定 JSON 字段。"""
    return {"id": job.id, "cron": job.cron, "prompt": job.prompt, "timezone": job.timezone, "recurring": job.recurring, "durable": job.durable, "identity": job.identity, "next_run_at_utc": job.next_run_at_utc.astimezone(dt.UTC).isoformat().replace("+00:00", "Z"), "last_slot_at_utc": None if job.last_slot_at_utc is None else job.last_slot_at_utc.astimezone(dt.UTC).isoformat().replace("+00:00", "Z")}
