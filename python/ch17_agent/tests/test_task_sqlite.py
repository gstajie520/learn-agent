...
from agent_ch17.features.tasks import CreateTaskInput, TaskGraphError, TaskStorageError
from agent_ch17.features.work_stealing import TaskClaimError, TaskLeaseExpiredError
...
def test_sqlite_preserves_creation_order_and_dependency_gate(tmp_path) -> None:  # type: ignore[no-untyped-def]
    store = SqliteTaskStore(str(tmp_path), id_generator=lambda: next(IDS), claim_token_generator=lambda: next(TOKENS))
...
def test_repeated_claim_token_rolls_back_second_claim(tmp_path) -> None:  # type: ignore[no-untyped-def]
    ...
    store = SqliteTaskStore(str(tmp_path), id_generator=lambda: next(ids), claim_token_generator=lambda: token)
