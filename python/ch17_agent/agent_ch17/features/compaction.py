...
class CompactionManager:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            preview_head_bytes < 0
            or preview_tail_bytes < 0
            or preview_head_bytes + preview_tail_bytes == 0
        ):
    ...
    def compact_tool_results(self, results: tuple[ToolResult, ...]) -> ToolResultBudgetOutcome:
        ...
        retained_bytes = sum(size for index, size in enumerate(sizes) if index not in selected)
        for index in ranked:
    ...
    def prepare(self, history: tuple[ChatMessage, ...]) -> tuple[ChatMessage, ...]:
        ...
        if (
            ...
            and self._prepared_history
            and history[: len(self._prepared_source)] == self._prepared_source
        ):
            prepared = self._prepared_history + history[len(self._prepared_source) :]
        prepared = snip_compact_history(
            prepared, self._snip_max_groups, self._snip_keep_head_groups
        )
    ...
    def compact_proactively(
        ...
        history: tuple[ChatMessage, ...],
        *,
        summary_history: tuple[ChatMessage, ...] | None = None,
    ...
    def compact_on_prompt_too_long(
        self, history: tuple[ChatMessage, ...], retry_count: int = 0
    ) -> HistoryCompactionOutcome:
    ...
    def _artifact_preview(self, reference: ArtifactReference, result: ToolResult) -> str:
        return _preview_content(
            reference, result.content, self._preview_head_bytes, self._preview_tail_bytes
        )
    ...
    def _write_artifact(self, kind: str, content: str, extension: str) -> ArtifactReference:
        ...
        return ArtifactReference(
            str(path),
            str(path.relative_to(self._workspace)).replace("\\", "/"),
...
def micro_compact_history(
    history: tuple[ChatMessage, ...], keep_recent_tool_groups: int = 3
) -> tuple[ChatMessage, ...]:
    groups = _groups(history)
    tool_indices = [index for index, group in enumerate(groups) if group.is_tool_exchange]
    compact_indices = set(
        tool_indices[:-keep_recent_tool_groups] if keep_recent_tool_groups else tool_indices
    )
...
def snip_compact_history(
    history: tuple[ChatMessage, ...], max_groups: int = 50, keep_head_groups: int = 3
) -> tuple[ChatMessage, ...]:
    groups = _groups(history)
    ...
    compacted = [
        *groups[:keep_head_groups],
        _MessageGroup((system_message(f"[Compacted: {omitted} message groups omitted]"),), False),
        *groups[-tail_count:],
    ]
