...
class ToolContext:
    ...
    idempotency_key: str | None = None  # 定时事件等外部触发的稳定幂等键。
    task_id: str | None = None  # 当前 claim 绑定的 Task UUID。
    claim_token: str | None = None  # 当前租约的完成凭证。
    worktree_name: str | None = None  # 当前 claim 对应的受管 Worktree 名称。
    execution_scope: object | None = None  # 一次 Agent.run 内共享的身份映射对象。
...
class ToolDefinition:
    ...
    concurrency: str | None = None  # 可选并发标记；P13 使用 background_eligible。
    source: str = "builtin"  # 工具来源；MCP 使用 mcp:{alias}:{remote_name}。
...
class ToolRegistry:
    ...
    def version(self) -> int:
        """返回动态工具集合版本；成功的整批变更只递增一次。"""
        return self._version

    def register(self, definition: ToolDefinition) -> ToolDefinition:
        """注册单个工具，并返回作为后续撤销凭据的同一对象。"""
        return self.register_many((definition,))[0]

    def register_many(
        self, definitions: tuple[ToolDefinition, ...] | list[ToolDefinition]
    ) -> tuple[ToolDefinition, ...]:
        """原子注册一批工具；任一冲突时一个也不写入。"""
        if not self._mutable:
            raise ValueError("工具注册表快照不可修改")
        batch = tuple(definitions)
        if not batch:
            return ()
        names = tuple(definition.name for definition in batch)
        if len(names) != len(set(names)):
            raise ValueError("工具批次中存在重名")
    ...
    def unregister_many(self, definitions: tuple[ToolDefinition, ...]) -> None:
        """按对象身份原子撤销工具，旧连接不能误删重连后的同名定义。"""
        if not self._mutable:
            raise ValueError("工具注册表快照不可修改")
        if not definitions:
            return
        if len({definition.name for definition in definitions}) != len(definitions):
            raise ValueError("撤销批次中存在重名")
        for definition in definitions:
            current = self._definitions.get(definition.name)
            if current is None:
                raise ValueError(f"工具没有注册: {definition.name}")
            if current is not definition:
                raise ValueError(f"工具定义与当前注册项不一致: {definition.name}")
        for definition in definitions:
    ...
    def snapshot(self) -> "ToolRegistry":
        ...
        """
        return ToolRegistry(self._definitions, mutable=False, version=self._version)
