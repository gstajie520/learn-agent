...
第二十章在此追加一个可选的尾部 ``runtime_status`` 段落。它承载“频繁变化但必须与
稳定段落分开”的运行态，例如 MCP 已连接哪些 alias、是否仍有异步工作。放在最后是为了
让状态变化不移动、不重排前面的稳定段落。
"""
...
# 运行态状态提供者：每次渲染前重新读取，而不是永久绑定构建期的 JSON 快照。
# Java 对照：类似注入一个 ``Supplier<Map<String, Object>>``，而不是注入一份 Map 常量。
DynamicPromptStatusProvider: TypeAlias = Callable[[], Mapping[str, object]]
...
class DynamicPromptRenderer:
    ...
    def render(
        ...
        *,
        ...
        ``status`` 是 P20 新增的可选尾部段落：它承载变化频繁但必须与稳定段落分开的
        运行态信息，因此始终追加在最后，状态变化不会移动前面段落的相对位置。
        """
        ...
        memory_body = "" if memory is None or not memory.selected else memory.render_selected()
        # 运行态状态与 context 走同一套 JSON 规范化，因此循环引用和非法值同样被拒绝。
        status_json = None if status is None else _stable_json(_normalize_context(status))
        # cache_input 是普通 dict，逐步装配后再统一序列化；Java 对照是先填 Map 再算 hash。
        cache_input: dict[str, JsonValue] = {
        ...
        if status_json is not None:
            # 只有存在状态时才写入缓存键，保证 P1-P19 的缓存键与迁移前完全一致。
            cache_input["status"] = status_json
        key = _stable_json(cache_input)
        ...
        if status_json is not None:
            # 运行态状态固定排在最后，状态变化不会移动或重排前面的稳定段落。
            sections.append(f"## runtime_status\n{status_json}")
        prompt = "\n\n".join(sections)
        self._last_key = key
...
class DynamicPromptProvider:
    ...
    def render(self) -> str:
        ...
        # 每轮重新调用 status_provider，而不是复用构建期快照，
        # 这样 MCP 连接和后台 pending work 的变化才能在下一轮 Prompt 中体现。
