...
class JsonProtocolStore:
    ...
    def __init__(
        ...
        workspace: str,
        *,
        # 两个可选注入点让测试可以固定 ID 和时间；Java 对照是注入 IdGenerator 与 Clock。
        # Callable[[], str] 表示“无参数、返回 str 的函数”，等价于 Java 的 Supplier<String>。
    ...
    def _parse(value: object) -> ProtocolRequest:
        ...
        # 用具名函数代替 lambda，mypy strict 才能推断出返回类型是 datetime。
        # Java 对照：等价于把内联匿名函数提取成一个私有静态方法。
        def parse_time(raw: object) -> dt.datetime:
...
class ProtocolTeamHost(Protocol):
    ...
    # 协议运行时只需要确认队友存在，不关心具体状态字段，因此返回 object。
    # 真实实现 TeammateRuntime.state() 返回 Teammate；这里不写具体类型是为了
    # 避免 protocol 反向 import teammates 形成循环依赖（teammates 已经 import protocol）。
