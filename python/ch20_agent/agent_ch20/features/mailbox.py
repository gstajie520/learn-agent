...
class ProtocolMailboxMessage:
    ...
    sender: str  # 发送方身份。
    recipient: str  # 接收方身份。
    kind: ProtocolMessageKind  # shutdown 或 plan approval 的请求/响应类型。
    content: str  # 展示给对方的正文或反馈。
    created_at_utc: dt.datetime  # 传输创建时间。
    request_id: str  # 关联 ProtocolRequest 的 UUID。
    approved: bool | None  # 请求必须为 None，响应必须为 bool。
...
@runtime_checkable
# runtime_checkable 让 isinstance 可以在运行期检查“方法是否存在”。
# Java 对照：Java 的 instanceof 天然能判断接口，Python 的 Protocol 默认只在静态检查期生效，
# 需要这个装饰器才允许运行期判断；它只核对方法名，不核对参数签名。
@runtime_checkable
class ProtocolMailboxStore(MailboxStore, Protocol):
    ...
    多出一个只有协议实现才提供的 ``send_protocol`` 方法。第十六章的协议运行时需要
    ``request_id``/``approved`` 这两个字段，普通 Mailbox 实现不需要。

    有了这个显式协议，``TeammateRuntime.deliver_protocol`` 就能用 ``isinstance``
    做一次类型收窄，而不必依赖 ``hasattr`` 加类型忽略注释；不支持协议的实现会在
    投递前得到明确报错，而不是在运行时抛出 ``AttributeError``。
    """
