...
def build_agent(
    ...
    if (
        ...
        and profile is not P17
        and profile is not P18
        and profile is not P19
    ):
    ...
    if profile in (P17, P18, P19) and work_stealing_runtime is None:
        raise ValueError("第十七章及以后必须提供 work_stealing_runtime")
    if profile not in (P17, P18, P19) and work_stealing_runtime is not None:
        raise ValueError("work_stealing_runtime 只适用于第十七章及以后")
    if profile in (P17, P18, P19):
        if task_store is None or work_stealing_runtime is None:
            raise ValueError("第十九章必须提供 SQLite task_store 和 work_stealing_runtime")
        if task_store is not work_stealing_runtime.store:
            raise ValueError("第十九章 task_store 必须和 work_stealing_runtime 共享同一实例")
    if profile in (P18, P19):
        if worktree_runtime is None:
            raise ValueError("第十九章必须提供 worktree_runtime")
        if task_store is not worktree_runtime.store:
            raise ValueError("第十九章 worktree_runtime 必须和 task_store 共享同一实例")
        if (
            work_stealing_runtime is None
            or work_stealing_runtime.claim_service is not worktree_runtime
        ):
            raise ValueError("第十九章 work_stealing_runtime 必须使用同一个 worktree_runtime")
    elif worktree_runtime is not None:
        raise ValueError("worktree_runtime 只适用于第十九章")
    if profile is P19 and mcp_runtime is None:
        raise ValueError("第十九章必须提供 mcp_runtime")
    if profile is not P19 and mcp_runtime is not None:
        raise ValueError("mcp_runtime 只适用于第十九章")
        ...
        if profile is P19:
            permission_rules.append(
                PermissionRule(
                    ...
                    "ask",
                    "第十九章外部 MCP 连接和 external 工具需要明确审批",
                    lambda request: (
                        profile is P19
                        and request.prepared.definition is not None
    ...
    if "subagent" in profile.capabilities:
        ...
        def child_tools_factory() -> tuple[ToolRegistry, TodoTracker]:
            ...
            if profile in (P17, P18, P19) and work_stealing_runtime is not None:
                register_leased_task_tools(
    ...
    if profile in (P18, P19) and worktree_runtime is not None:
        for definition in worktree_runtime.lead_tool_definitions:
            tools.register(definition)
    elif profile not in (P17, P18, P19) and task_store is not None:
        # 五个 Task 工具最后追加，P11 的工具列表保持完整前缀。
        register_task_tools(tools, cast(TaskStore, task_store))
    if profile is P19 and mcp_runtime is not None:
    ...
    if profile in (P14, P15, P16, P17, P18, P19):
    ...
    if profile in (P15, P16, P17, P18, P19):
        ...
        def teammate_factory(name: str, role: str, send_definition: object) -> AgentRunner:
            ...
            return AgentRunner(
                ...
                tool_context_provider=worktree_runtime if profile in (P18, P19) else None,
            )
