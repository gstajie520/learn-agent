"""第十九章 SQLite 任务仓库：保存 DAG、认领令牌和有限租约。
...
class SqliteTaskStore:
    """把第十九章项目任务保存到 ``.agent_tutorial/tasks.sqlite3``。
            ...
            try:
                connection.execute(
                    ...
                    (
                        ...
                        _encode_time(binding.created_at_utc),
                        _encode_time(binding.updated_at_utc),
                    ),
                )
    ...
    def activate_worktree(
        ...
        return self._transition_worktree(
            task_id, ("reserved",), "active", "create", branch_tip, None, occurred_at_utc, "pending"
        )
    ...
    def keep_worktree(
        ...
        return self._transition_worktree(
            task_id, ("active",), "kept", "keep", branch_tip, None, occurred_at_utc, "completed"
        )
        ...
        with self._transaction() as connection:
            ...
            active = {
                ...
                for row in connection.execute(
                    "SELECT task_id FROM worktree_bindings WHERE status='active'"
                )
            }
            ...
            for task in tasks:
                if (
                    ...
                    and all(by_id[d].status == "completed" for d in task.blocked_by)
                ):
    ...
    def lookup_claim(self, claim_token: str) -> TaskClaim | None:
        ...
        with self._transaction() as connection:
            known = connection.execute(
                "SELECT task_id FROM task_claim_tokens WHERE token=?", (token,)
            ).fetchone()
            if known is None:
                return None
            now = self._now()
            ...
            row = connection.execute(
                "SELECT * FROM tasks WHERE claim_token=? AND status='in_progress'", (token,)
            ).fetchone()
            if row is None:
    ...
    def complete_task(self, task_id: str, owner: str, claim_token: str) -> TaskCompletion:
        ...
        with self._transaction() as connection:
            ...
            unblocked = tuple(
                ...
                and all(
                    by_id[dependency].status == "completed" for dependency in candidate.blocked_by
                )
            )
    ...
    def _claim(
        ...
        claimed = Task(
            task.id, task.subject, task.description, "in_progress", owner, task.blocked_by
        )
        ...
        with self._transaction() as connection:
            ...
            changed = connection.execute(
                ...
                (
                    ...
                    _encode_time(updated.updated_at_utc),
                    updated.task_id,
                    current.status,
                ),
            ).rowcount
    ...
    def _load_worktree_binding(connection: sqlite3.Connection, task_id: str) -> WorktreeBinding:
        ...
        row = connection.execute(
            "SELECT * FROM worktree_bindings WHERE task_id=?", (task_id,)
        ).fetchone()
        if row is None:
    ...
    def _append_worktree_event(
        connection: sqlite3.Connection, action: str, binding: WorktreeBinding
    ) -> None:
