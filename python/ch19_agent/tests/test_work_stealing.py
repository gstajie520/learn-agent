...
from agent_ch19.features.tasks import CreateTaskInput, Task, TaskCompletion
from agent_ch19.features.work_stealing import (
    TaskClaim,
    WorkStealingRuntime,
...
def test_create_tool_maps_blocked_by_and_claim_prompt_contains_lease() -> None:
    ...
    call = tools.prepare(
        __import__("agent_ch19.core.messages", fromlist=["tool_call"]).tool_call(
            "call-1",
            "create_task",
            json.dumps(
                {
    ...
    task = Task("00000000-0000-4000-8000-000000001701", "任务", "", "in_progress", "alice", ())
