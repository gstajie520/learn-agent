...
from agent_ch08.core.tools import tool_error, tool_success
from agent_ch08.features.compaction import (
...
class RecordingSummarizer:
    ...
    def summarize(self, history):
        ...
        return CompactionSummary(
            "继续迁移第八章",
            ("按完整消息组压缩",),
            ("agent_ch08/features/compaction.py",),
            ("运行测试",),
            ("不能拆散工具消息",),
        )
...
def test_proactive_compaction_persists_transcript_then_returns_structured_summary(tmp_path: Path):
    ...
    assert summary["current_goal"] == "继续迁移第八章"
