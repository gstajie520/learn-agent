...
这是什么：定义 Agent 与模型之间交互的消息类型
Java 类比：record SystemMessage(...), record UserMessage(...), etc.
为什么需要：类型安全地表示不同角色的消息，确保工具调用和结果正确配对
...
class SystemMessage:
    ...
    为什么需要：设定模型的角色和行为准则，引导模型按预期方式工作
