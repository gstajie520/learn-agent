...
from agent_ch06.core.profiles import P06
from agent_ch06.features.subagents import DEFAULT_SUBAGENT_SYSTEM_PROMPT
...
def test_child_cannot_escape_workspace_even_when_approval_allows(tmp_path: Path) -> None:
    outside = tmp_path.parent / "outside-ch06.txt"
    outside.write_text("sentinel", encoding="utf-8")
    parent_call = tool_call("parent-task", "task", '{"description":"write outside"}')
    child_write = tool_call(
        "outside-write", "write_file", '{"path":"../outside-ch06.txt","content":"changed"}'
    )
    parent = FakeModel(
        [
            ModelReply(assistant_message(None, (parent_call,)), "tool_calls"),
