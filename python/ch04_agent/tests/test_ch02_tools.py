...
from agent_ch04.adapters.filesystem import LocalWorkspaceFileSystem
from agent_ch04.bootstrap import build_agent
from agent_ch04.core.commands import CommandResult
from agent_ch04.core.messages import assistant_message, tool_call, validate_tool_pairing
from agent_ch04.core.model import ModelReply
from agent_ch04.core.profiles import P01, P02, ChapterProfile
from agent_ch04.features.builtin_tools import create_chapter_two_tools
...
def test_p01_and_p02_tool_sets_are_separate(tmp_path: Path) -> None:
    p01 = build_agent(P01, FakeModel([ModelReply(assistant_message("ok"), "stop")]), str(tmp_path), command_runner=FakeCommandRunner())
    p02_model = FakeModel([ModelReply(assistant_message("ok"), "stop")])
    p02 = build_agent(P02, p02_model, str(tmp_path), command_runner=FakeCommandRunner(), file_system=LocalWorkspaceFileSystem())
    p01.run("ok")
    p02.run("ok")
    assert [tool.name for tool in p01._tools.snapshot().openai_tools()] == ["shell"]
    assert [tool.name for tool in p02._tools.snapshot().openai_tools()] == ["shell", "read_file", "write_file", "edit_file", "glob"]
...
def test_model_can_write_edit_read_and_glob_in_one_turn(tmp_path: Path) -> None:
    model = FakeModel([
        ModelReply(assistant_message(None, (
            ...
            tool_call("edit", "edit_file", '{"path":"note.txt","old_text":"alpha","new_text":"gamma"}'),
            tool_call("read", "read_file", '{"path":"note.txt"}'),
            tool_call("glob", "glob", '{"pattern":"**/*.txt"}'),
        )), "tool_calls"),
        ModelReply(assistant_message("完成。"), "stop"),
    ])
    runner = build_agent(P02, model, str(tmp_path), command_runner=FakeCommandRunner(), file_system=LocalWorkspaceFileSystem())
...
def test_extra_fields_are_rejected_before_file_side_effect(tmp_path: Path) -> None:
    ...
    result = registry.invoke(registry.prepare(tool_call("x", "read_file", '{"path":"x","extra":true}')), type("Context", (), {"workspace": str(tmp_path), "identity": "test"})())
