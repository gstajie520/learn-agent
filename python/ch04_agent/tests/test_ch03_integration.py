...
from agent_ch04.core.permissions import PermissionDecision, PermissionRequest
from agent_ch04.core.profiles import P02, P03
...
def model_for_write(path: str = "note.txt") -> FakeModel:
    return FakeModel([
        ModelReply(assistant_message(None, (tool_call("write", "write_file", f'{{"path":"{path}","content":"chapter three"}}'),)), "tool_calls"),
...
def test_only_p03_adds_write_approval_and_audit(tmp_path: Path) -> None:
    ...
    build_agent(P02, model_for_write(), str(tmp_path), command_runner=FakeCommandRunner(), file_system=fs, approval_provider=p02_approval, audit_sink=p02_audit).run("写文件")
    assert p02_approval.requests == [] and p02_audit.decisions == []
    (tmp_path / "note.txt").unlink()
    approval, audit = Approval(), Audit()
    result = build_agent(P03, model_for_write(), str(tmp_path), command_runner=FakeCommandRunner(), file_system=fs, approval_provider=approval, audit_sink=audit).run("写文件")
...
def test_audit_failure_returns_paired_error_and_skips_handler(tmp_path: Path) -> None:
    result = build_agent(P03, model_for_write(), str(tmp_path), file_system=LocalWorkspaceFileSystem(), approval_provider=Approval(), audit_sink=Audit(fail=True)).run("写文件")
