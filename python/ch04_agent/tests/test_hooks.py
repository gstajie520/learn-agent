...
from agent_ch04.core.hooks import (
    HOOK_EVENTS,
    HookContext,
    HookContractError,
    HookRegistry,
...
def prepared_call(value: int = 1):
    ...
    registry.register(ToolDefinition("echo", "回显整数", {"type": "object"}, "read", lambda _args, _ctx: tool_success("不应执行"), lambda args: set(args) == {"value"} and isinstance(args["value"], int)))
...
def test_pre_blocking_short_circuits_later_callbacks() -> None:
    calls: list[str] = []
    hooks = HookRegistry()
    hooks.register("PreToolUse", lambda _context: calls.append("block") or HookResult(blocking_error=tool_error("hook_blocked", "已阻止")))
...
def test_pre_rewrite_flows_to_later_callback_and_is_immutable() -> None:
    ...
    hooks = HookRegistry()
    hooks.register("PreToolUse", lambda _context: HookResult(updated_input=replace(original, call=tool_call("call-echo", "echo", '{"value":2}'), arguments={"value": 2})))
    hooks.register("PreToolUse", lambda context: observed.append(context.prepared.arguments if context.prepared else None) or HookResult())
...
def test_pre_rewrite_cannot_change_trusted_identity_or_schema(change: str) -> None:
    ...
    if change == "id":
        updated = replace(original, call=tool_call("changed", "echo", '{"value":2}'), arguments={"value": 2})
    elif change == "name":
        updated = replace(original, call=tool_call("call-echo", "other", '{"value":2}'), arguments={"value": 2})
...
def test_post_rewrites_chain_and_permission_uses_strictest_value() -> None:
    observed: list[str] = []
    hooks = HookRegistry()
    hooks.register("PostToolUse", lambda context: observed.append(context.result.content) or HookResult(updated_output=tool_success("第一次")))
    hooks.register("PostToolUse", lambda context: observed.append(context.result.content) or HookResult(updated_output=tool_success("第二次")))
    ...
    for behavior in ("allow", "ask", "deny"):
        permission_hooks.register("PreToolUse", lambda _context, value=behavior: HookResult(permission_behavior=value))  # type: ignore[arg-type]
...
def test_stop_can_force_only_one_continuation_but_callback_runs_twice() -> None:
    states: list[bool] = []
    hooks = HookRegistry()
    hooks.register("Stop", lambda context: states.append(context.stop_hook_active) or HookResult(force_continue=user_message("再核对一次")))
...
def test_user_prompt_accepts_only_system_additional_context() -> None:
    hooks = HookRegistry()
    hooks.register("UserPromptSubmit", lambda _context: HookResult(additional_context=(system_message("项目规则"),)))
    assert run(hooks.run_user_prompt(user_message("开始"))).additional_context == (system_message("项目规则"),)
