...
class ModelMemoryQueries:
    ...
    def _complete(self, instruction: str, messages: tuple[ChatMessage, ...]) -> str:
        ...
        if reply.finish_reason != "stop":
            raise MemoryStoreError(f"记忆 side-query 必须以 stop 结束，实际为 {reply.finish_reason}")
...
class MemoryStore:
    ...
    def extend(self, records: Sequence[MemoryRecord]) -> None:
        ...
        with self._lock():
            ...
            added = [
                _StoredMemory(self._new_filename(record.name), record) for record in validated
            ]
    ...
    def _commit(
        ...
        *,
        ...
        manifest = _stable_json(
            {"version": 1, "files": [item.filename for item in records]}
        )
...
class MemorySession:
            ...
            if self._selector is not None:
                try:
                    ...
                    self._selected = tuple(
                        by_name[name] for name in names[: self._max_selected]
                    )
                    return
    ...
    def render_selected(self) -> str:
        ...
        for record in self._selected:
            sections.extend(
                [f"## {record.name} ({record.kind})", record.description, record.body]
            )
    ...
    def complete(self, history: tuple[ChatMessage, ...]) -> None:
        ...
        try:
            ...
            try:
                ...
                self._store.apply_consolidation(
                    current, extracted, plan.source_names, plan.records
                )
            except Exception:  # noqa: BLE001
...
def _is_windows_reserved(value: str) -> bool:
    ...
    return component in WINDOWS_RESERVED_NAMES or bool(WINDOWS_RESERVED_PATTERN.fullmatch(component))
...
def _parse_records(output: str, allow_empty: bool) -> tuple[MemoryRecord, ...]:
    ...
    for item in cast(list[object], value):
        ...
        if not all(isinstance(item[field], str) for field in ("name", "type", "description", "body")):
...
def _parse_consolidation_plan(
    ...
    if not isinstance(raw_sources, list) or not all(
        isinstance(name, str) for name in raw_sources
    ):
...
def _keyword_select(
    ...
    ranked = sorted(
        (
            (
                sum(
                    token in f"{record.name} {record.description}".lower()
                    for token in tokens
                ),
                record,
            )
