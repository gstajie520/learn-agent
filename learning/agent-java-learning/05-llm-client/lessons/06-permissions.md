...
| 文件 | 作用 |
| --- | --- |
| `lesson06/PermissionBehavior.java` | 四态枚举，`isFinal()` 区分中间态 |
| `lesson06/PermissionDecision.java` | 决定 + `reason` + `source`，三者都不可为空 |
| `lesson06/PermissionRequest.java` | 提交裁决的请求，带 `PreparedToolCall` 而非原始调用 |
| `lesson06/PermissionRule.java` | 一条规则：匹配器 + 行为 + 理由 |
| `lesson06/ApprovalProvider.java` | 人工裁决接口 |
| `lesson06/AuditSink.java` | 审计接口，允许抛异常 |
| `lesson06/PermissionPolicy.java` | 候选收集 + 三轮归约 + 审批收敛 + 审计 |
| `lesson06/GuardedTrace.java` | 轨迹 + 裁决记录 |
| `lesson06/GuardedAgentLoop.java` | 把裁决插进边界链 |
| `lesson06/PermissionDemo.java` | 五个场景 |

测试 36 个，全部离线：

```powershell
mvn -o -pl 05-llm-client test "-Dtest=PermissionPolicyTest,GuardedAgentLoopTest"
```
