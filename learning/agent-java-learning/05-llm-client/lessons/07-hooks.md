...
| 文件 | 作用 |
| --- | --- |
| `lesson07/HookEvent.java` | 四个事件的枚举 |
| `lesson07/HookContext.java` | 事件上下文，按事件校验字段归属 |
| `lesson07/HookResult.java` | Hook 的返回，setter 即校验 |
| `lesson07/HookCallback.java` | 单方法回调接口 |
| `lesson07/HookRegistry.java` | 注册、串行执行、合并、三道锁 |
| `lesson07/HookContractException.java` | 契约违反的稳定异常类型 |
| `lesson07/HookedAgentLoop.java` | 把四个事件插进循环 |
| `lesson07/HookDemo.java` | 六个场景 |

测试 33 个，全部离线：

```powershell
mvn -o -pl 05-llm-client test "-Dtest=HookRegistryTest,HookedAgentLoopTest"
```
...
java "-Dfile.encoding=UTF-8" -cp '05-llm-client/target/classes;05-llm-client/target/dependency/*' learn.agent.llm.lesson07.HookDemo
```
