...
public final class HookRegistry {
    ...
    public HookResult runUserPrompt(learn.agent.llm.lesson01.ChatMessage message) {
    ...
    public HookResult runPostTool(PreparedToolCall prepared,
                                  learn.agent.llm.lesson04.ToolExecutionResult result) {
        return run(HookContext.postToolUse(prepared, result));
    }

    /** Stop：模型给出最终答复、准备返回之前。 */
    public HookResult runStop(List<learn.agent.llm.lesson01.ChatMessage> history, boolean stopHookActive) {
