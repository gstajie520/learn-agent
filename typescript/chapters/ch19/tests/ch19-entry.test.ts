...
import { P19 } from "../src/core/profiles.js";

describe("chapter 19 fixed entry", () => {
  test("reports missing settings before Git or MCP state", async () => {
    const root = await mkdtemp(join(tmpdir(), "agent-tutorial-ch19-entry-"));
    const original = process.cwd();
    const write = vi.spyOn(process.stderr, "write").mockImplementation(() => true);
    try {
      process.chdir(root);
      await expect(runProfile(P19, ["--prompt", "hello"])).resolves.toBe(2);
  ...
  test("closes the model before rejecting a valid non-Git workspace", async () => {
    ...
    try {
      ...
      await expect(runCli(["run", "--chapter", "19", "--prompt", "hello"])).resolves.toBe(1);
