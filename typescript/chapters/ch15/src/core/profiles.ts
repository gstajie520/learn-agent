...
export type Capability =
  ...
  | "background"
  // cron 表示可持久化五字段 Cron 计划并由后台 worker 轮询触发。
  | "cron"
  // teammate 与 mailbox 共同启用身份隔离的协作 Agent 和持久异步通信。
  | "teammate"
  | "mailbox";
...
// P14 在 P13 能力上追加 cron，由持久 Cron 计划触发上下文事件回合。
export const P14: ChapterProfile = Object.freeze({
  chapter: 14,
  capabilities: new CapabilitySet([...P13.capabilities, "cron"]),
});

// P15 在 P14 能力上追加 teammate 与 mailbox，让 Lead 与持久队友通过共享事件和邮箱协作。
