...
describe("chapter 18 worktrees", () => {
  ...
  test("keeps a kept binding as kept when removal fails", async () => {
    for (const runner of [
      new FailingStatusGitRunner(new SubprocessGitRunner()),
      new RaisingStatusGitRunner(new SubprocessGitRunner()),
    ]) {
      const { root, store, runtime } = await components(runner);
      const task = await store.createTask({ subject: "kept failure", description: "" });
      const active = await runtime.createWorktree({
        taskId: task.id,
        name: runner instanceof FailingStatusGitRunner ? "keptnonzero" : "keptprocess",
        integrationRef: "refs/heads/main",
      });
      await complete(runtime, task.id, "alice");
      await expect(runtime.keepWorktree(task.id)).resolves.toMatchObject({
