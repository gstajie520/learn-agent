...
describe("AgentRunner request and tool-result processors", () => {
  ...
  test("begins a recovery executor once for a multi-request tool turn", async () => {
    const calls = [toolCall("call-1", "missing", "{}")];
    const replies = [
      { message: assistantMessage(null, calls), finishReason: "tool_calls" as const },
      { message: assistantMessage("done"), finishReason: "stop" as const },
    ];
    const executor = {
      beginCalls: 0,
      requests: [] as unknown[],
    ...
    expect(executor.beginCalls).toBe(1);
