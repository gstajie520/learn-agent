...
describe("dynamic prompt renderer", () => {
  ...
  test("re-reads a live status factory and appends it after memory", () => {
    const tools = new ToolRegistry();
    const renderer = new DynamicPromptRenderer();
    let connected = false;
    const provider = new DynamicPromptProvider({
      ...
      context: { chapter: 20 },
      statusProvider: () =>
        Object.freeze({ mcp_connections: Object.freeze(connected ? ["fake"] : []) }),
    });
    ...
    expect(first).toContain('## runtime_status\n{"mcp_connections":[]}');
    expect(second).toContain('## runtime_status\n{"mcp_connections":["fake"]}');
    expect(first.indexOf("## runtime_status")).toBeGreaterThan(first.indexOf("## tools"));
    expect(second.endsWith('## runtime_status\n{"mcp_connections":["fake"]}')).toBe(true);
    expect(renderer.cacheHits).toBe(0);
    expect(provider.render()).toBe(second);
    expect(renderer.cacheHits).toBe(1);
  });

  test("rejects a non-function live status provider", () => {
    const common = {
    ...
    expect(
      () =>
        new DynamicPromptProvider({
          ...common,
          statusProvider: "not a function" as never,
        }),
    ).toThrow(/statusProvider must be a function/);
  });
