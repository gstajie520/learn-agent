...
describe("chapter profiles", () => {
  ...
  test("the capability collection has no mutation API and profile lookup is fixed", () => {
    ...
    expect(profileForChapter(15)).toBe(P15);
    expect([...P15.capabilities]).toEqual([...P14.capabilities, "teammate", "mailbox"]);
    expect(profileForChapter(16)).toBe(P16);
    expect([...P16.capabilities]).toEqual([...P15.capabilities, "protocol", "plan_gate"]);
    ...
    expect([...P20.capabilities]).toEqual([...P19.capabilities, "full_harness"]);
  });

  test.each([0, 21, 1.5, Number.NaN])("rejects invalid chapter %s", (chapter) => {
    expect(() => profileForChapter(chapter)).toThrow("chapter must be an integer from 1 to 20");
  });
});
