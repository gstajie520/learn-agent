...
describe("chapter 13 bootstrap", () => {
  test("requires a background supervisor and adds P13-only background tools", async () => {
    ...
    try {
      ...
      const runner12 = buildAgent(P12, { ...common, model: model12 });
      await runner12.run("inspect tools");
      const p12Names = model12.requests[0]?.tools.map((tool) => tool.function.name) ?? [];
      expect(p12Names).not.toContain("query_background_job");
      expect(p12Names).not.toContain("cancel_background_job");
      model12.assertExhausted();
      await runner12.close();
      ...
      const p13Names = model13.requests[0]?.tools.map((tool) => tool.function.name) ?? [];
      expect(p13Names).toContain("query_background_job");
      expect(p13Names).toContain("cancel_background_job");
