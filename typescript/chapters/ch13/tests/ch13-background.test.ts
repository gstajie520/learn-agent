...
describe("chapter 13 background runtime", () => {
  ...
  test("background job tools query persisted state and cancel running jobs", async () => {
    const root = await workspace();
    try {
      const store = new JsonBackgroundJobStore(root);
      const inbox = new EventInbox();
      ...
      const supervisor = new JobSupervisor({
        ...
        eventIdGenerator: new SequenceId(EVENT_ID, EVENT_ID_2).next,
      });
      const jobId = await supervisor.submit({
        sourceToolCallId: "query-call",
        toolName: "shell",
        operation: async () => toolSuccess("compiled"),
      });
      const tools = new ToolRegistry();
      registerBackgroundJobTools(tools, supervisor);
      const context = Object.freeze({ workspace: root, identity: "user" });

      const running = await tools.invoke(
        tools.prepare(
          toolCall("query-running", "query_background_job", JSON.stringify({ job_id: jobId })),
        ),
        context,
      );
      expect(running.isError).toBe(false);
      expect(JSON.parse(running.content)).toMatchObject({
        job_id: jobId,
        status: BackgroundJobStatus.RUNNING,
        result: null,
      });

      const cancelled = await tools.invoke(
        tools.prepare(
          toolCall("cancel-call", "cancel_background_job", JSON.stringify({ job_id: jobId })),
        ),
        context,
      );
      ...
      expect(JSON.parse(cancelled.content)).toMatchObject({
        ...
        result: { error_code: "background_cancelled", is_error: true },
      });
      expect((await store.getJob(jobId)).status).toBe(BackgroundJobStatus.CANCELLED);
      ...
      expect(missing.errorCode).toBe("background_job_not_found");
      await supervisor.close();
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  ...
  test("batch event drain injects every terminal event with position metadata", async () => {
    const root = await workspace();
    try {
      const store = new JsonBackgroundJobStore(root);
      const inbox = new EventInbox();
      ...
      const result = await runner.run("start");
      const eventMessages = model.requests[0]?.messages.filter(
        (message) =>
          message.role === "user" &&
          typeof message.content === "string" &&
          message.content.includes('"kind":"background_job"'),
      );
      expect(eventMessages).toHaveLength(2);
      const eventJobs = eventMessages?.map((message) => {
        const payload = JSON.parse(String(message.content)) as {
          batch: { index: number; total: number };
          runtime_event: { job_id: string };
        };
        expect(payload.batch.total).toBe(2);
        return payload.runtime_event.job_id;
      });
      expect(new Set(eventJobs)).toEqual(new Set([JOB_ID, JOB_ID_2]));
      eventJobs?.forEach((jobId, index) => {
        const message = eventMessages?.[index];
        const payload = JSON.parse(String(message?.content)) as {
          batch: { index: number; total: number };
          runtime_event: { job_id: string };
        };
        expect(payload.batch.index).toBe(index);
        expect(payload.runtime_event.job_id).toBe(jobId);
      });
