...
OpenAI SDK 位于 `typescript/chapters/ch01/src/adapters/openai-chat.ts`：适配器使用 Chat Completions 发出请求，再立即把 SDK 响应转成 `ModelReply` 和内部消息。**核心循环本身不保存 SDK 对象**——这条边界在第 3 步会再提一次，它是整套代码能长期维护的原因之一。

shell 的输入 schema、handler 和副作用类别来自同一个 `ToolDefinition`。下面是与配套实现对应的核心定义；实际源码位于 `typescript/chapters/ch01/src/features/builtin-tools.ts`：
...
完整版本在 `typescript/chapters/ch01/src/core/loop.ts`，第 1 章固定入口在 `typescript/chapters/ch01/src/chapters/ch01.ts`，行为测试在 `typescript/chapters/ch01/tests/ch01-loop.test.ts` 和 `typescript/chapters/ch01/tests/shell-tool.test.ts`。
...
在 `typescript/chapters/ch01/tests/ch01-loop.test.ts` 里找到构造 Agent 的地方，把 `maxTurns` 传成 `1`，然后让脚本化模型第一轮就请求工具。
