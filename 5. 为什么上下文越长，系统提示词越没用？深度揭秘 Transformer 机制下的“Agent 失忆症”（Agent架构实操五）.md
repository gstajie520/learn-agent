...
| 能跑起 `typescript/` 里的测试 | 第 1 章 | `npm ci` 已执行 |
...
实现位于 `typescript/chapters/ch05/src/features/todos.ts`。先定义允许的状态和数量上限：
...
和前四章完全一样：`typescript/` 里 `npm ci`，`.env` 里填好三个变量。
...
- `typescript/chapters/ch05/src/features/todos.ts`：Zod 契约、会话快照和 reminder 计数；
- `typescript/chapters/ch05/src/core/loop.ts`：模型调用前读取 guidance，工具轮后记录工具名；
- `typescript/chapters/ch05/src/bootstrap.ts`：每个 P05 会话创建并注册一个 tracker；
- `typescript/chapters/ch05/src/core/profiles.ts`：新增固定 `P05` 和 `todo` capability；
- `typescript/chapters/ch05/src/chapters/ch05.ts`：第五章固定入口；
- `typescript/chapters/ch05/tests/todos.test.ts`：TODO 领域行为和失败边界；
- `typescript/chapters/ch05/tests/ch05-todos.test.ts`：从组合根验证权限、消息和提醒接缝。
