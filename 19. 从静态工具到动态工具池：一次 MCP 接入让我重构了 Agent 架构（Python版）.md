...
本章内置 server 位于 [demo.ts](typescript/chapters/ch19/src/mcp-servers/demo.ts)，两个 alias 启动同一个模块，只传入不同 label：
...
领域策略与动态注册在 [mcp-tools.ts](typescript/chapters/ch19/src/features/mcp-tools.ts)，官方 SDK adapter 在 [mcp-client.ts](typescript/chapters/ch19/src/adapters/mcp-client.ts)。组合根和固定入口分别见 [bootstrap.ts](typescript/chapters/ch19/src/bootstrap.ts)、[cli.ts](typescript/chapters/ch19/src/cli.ts) 与 [ch19.ts](typescript/chapters/ch19/src/chapters/ch19.ts)。
...
- [Registry Snapshot 测试](typescript/chapters/ch19/tests/ch19-registry.test.ts)
- [真实 stdio、schema、权限与故障测试](typescript/chapters/ch19/tests/ch19-mcp.test.ts)
- [Lead/Subagent/Teammate 组合测试](typescript/chapters/ch19/tests/ch19-bootstrap.test.ts)
- [固定入口、allowlist 与关闭测试](typescript/chapters/ch19/tests/ch19-entry.test.ts)
